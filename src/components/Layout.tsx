...
const Layout = () => {
  ...
  const settingsClasses = cn(
    settings.lowStimulusMode && "low-stimulus",
    settings.highContrastMode && "high-contrast",
    settings.textSize === 'large' && "text-lg",
    settings.textSize === 'larger' && "text-xl"
  );

  return (
    <div className={cn("min-h-screen bg-background", settingsClasses)}>
      ...
      <header className="sticky top-0 z-40 w-full border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80">
        <div className="container flex h-16 items-center justify-between">
          <NavLink 
            ...
            aria-label="Values Compass - Home"
          >
      ...
      <footer className="border-t border-border bg-muted/30 py-8" role="contentinfo">
        <div className="container text-center text-sm text-muted-foreground">
          ...
          <p className="mt-2">All your data stays on your device. No accounts required.</p>
        </div>
      </footer>
    </div>
  );
